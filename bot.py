...
# ============================================================
# MAX ACTIVE TRADES - NEW!
# ============================================================

MAX_ACTIVE_TRADES = 3  # Only 3 trades at a time

# ============================================================
# ENHANCED INTELLIGENCE CLASS (ORIGINAL v8.1)
# ============================================================
...
# ============================================================
# SMART COIN SELECTOR (ORIGINAL v8.1)
# ============================================================
...
# ============================================================
# POSITION SIZING - FIXED FOR 30% PORTFOLIO
# ============================================================

MIN_LOT_SIZE = 0.001
LOT_SIZE_STEP = 0.001
PORTFOLIO_ALLOCATION = 0.30  # 30% of portfolio per trade
...
def calculate_smart_position_size(capital, kelly_fraction, atr, current_price, confidence_score=1.0):
    """
    Calculate position size based on 30% portfolio allocation.
    Size = (Capital * 0.30) / Current Price
    """
    try:
        ...
        # Take min of all constraints, but favor portfolio allocation
        ...
        final_position = max(MIN_LOT_SIZE, final_position)
        
        logger.info(f"📊 Position Size: {final_position:.4f} (30% of ${capital:.2f} = ${portfolio_allocation_amount:.2f})")
...
def send_telegram_photo(photo_data, caption=""):
    ...
    try:
        ...
        logger.debug(f"Telegram photo error: {e}")
        return False

# ============================================================
# TRADE FILTERS - IMPROVED FOR BETTER WIN RATE
# ============================================================
...
def get_statistics(symbol=None):
    """Get trading statistics with proper win rate calculation. Supports per-symbol stats."""
    try:
        with db_manager.get_connection() as conn:
            cursor = conn.cursor()
            ...
            if symbol:
                cursor.execute("""
                    ...
                    FROM trades 
                    WHERE status IN ('WIN', 'LOSS') AND symbol = ?
                """, (symbol,))
            else:
                cursor.execute("""
                    SELECT 
...
def calculate_dynamic_sl_tp(current_price, atr, side):
    """Calculate wider SL and TP for better win rate"""
    if atr is None or atr <= 0:
        atr = current_price * 0.01 if current_price > 0 else 1.0
    ...
    if ATR_DYNAMIC_ADJUSTMENT:
        ...
        if atr_percent < 0.5:
            sl_multiplier = ATR_SL_MULTIPLIER_BASE * 0.9
            tp_multiplier = ATR_TP_MULTIPLIER_BASE * 0.9
        elif atr_percent > 2.0:
            sl_multiplier = ATR_SL_MULTIPLIER_BASE * 1.2
            tp_multiplier = ATR_TP_MULTIPLIER_BASE * 1.1
        else:
...
# ============================================================
# MANAGE TRADES - IMPROVED PNL CALCULATION WITH BALANCE UPDATE
# ============================================================
...
def manage_trades():
    ...
    for symbol in list(active_trades.keys()):
        try:
            ...
            # Update position data with real-time prices
            ...
            # Update current capital with PnL
            if TRADING_MODE == "PAPER":
                current_capital = INITIAL_CAPITAL + sum([
                    get_trade_pnl(sym) for sym in active_trades.keys()
                ])
            ...
            if side == "BUY":
                if current_price >= tp:
                    ...
                    consecutive_losses = 0
                    safe_telegram_send(f"🎯 WIN {symbol}: +${profit:.4f} (Leverage: {leverage}x)")
                    ...
                    safe_telegram_send(f"🛑 LOSS {symbol}: -${loss:.4f} (Leverage: {leverage}x)")
...
def get_trade_pnl(symbol):
    """Get current PnL for a trade"""
    if symbol in position_data:
        return position_data[symbol].get('pnl', 0)
    return 0

# ============================================================
# ANALYZE AND TRADE - SIGNALS NEVER STOP & PERFECT R:R
# ============================================================
...
def analyze_and_trade():
    ...
    try:
        ...
        # Smart coin selection (Analysis NEVER stops)
        ...
        # Check global win rate threshold
        win_rate_ok = True
        if total_trades >= MIN_TRADES_FOR_WIN_RATE:
            if win_rate < MIN_WIN_RATE:
                win_rate_ok = False
                logger.info(f"⏭️ Global Win Rate {win_rate:.1f}% < {MIN_WIN_RATE}% - Reducing new trades")
            else:
                logger.info(f"✅ Global Win Rate OK: {win_rate:.1f}% >= {MIN_WIN_RATE}%")
        else:
        ...
        # Process symbols - Analysis runs for ALL, even if max trades reached
        for symbol in trading_symbols[:20]:  # Check top 20 coins
            ...
            try:
                ...
                # ============================================================
                # SIGNAL DETECTED - ALWAYS SEND TO TELEGRAM, EVEN IF TRADES FULL!
                # ============================================================
                ...
                # DYNAMIC SL/TP Golden Ratio
                sl, tp, sl_mult, tp_mult = calculate_dynamic_sl_tp(current_price, atr, side)
                
                # CONFIDENCE BOOST: If Confidence > 80%, we go for 1:3 (Maza double!)
                # If Confidence < 55%, we go for 1:1.5 (Safe mode)
                risk_dist = abs(current_price - sl)
                reward_dist = abs(tp - current_price)
                
                if confidence_score > 80:
                    tp = current_price + (atr * 6.0) if side == "BUY" else current_price - (atr * 6.0)
                elif confidence_score < 55:
                    tp = current_price + (atr * 3.0) if side == "BUY" else current_price - (atr * 3.0)

                kelly_fraction = calculate_dynamic_kelly()
                
                # Calculate position size - USING 30% PORTFOLIO
                size = calculate_smart_position_size(portfolio_value, kelly_fraction, atr, current_price, confidence_factor)

                # Calculate leverage
                leverage = min(MAX_LEVERAGE, max(MIN_LEVERAGE, int(confidence_score / 15) + 1))

                # GET EXACT COIN WIN RATE
                coin_stats = get_statistics(symbol)
                coin_win_rate = coin_stats.get('win_rate', 0)
                coin_trades = coin_stats.get('total', 0)

                # Final R:R Calculation
                final_risk_dist = abs(current_price - sl)
                final_reward_dist = abs(tp - current_price)
                rr_ratio = final_reward_dist / final_risk_dist if final_risk_dist > 0 else 0

                signal_text = (
                    f"🐋 *WHALE INSTITUTE SIGNAL: {symbol}*\n"
                    f"🔹 Action: `{side}`\n"
                    ...
                    f"📈 R:R Ratio: `1 : {rr_ratio:.1f}` (Golden 1:2+)\n"
                    f"📊 {symbol} WR: `{coin_win_rate:.1f}%` ({coin_trades} trades)\n"
                    f"🌐 Global WR: `{win_rate:.1f}%` ({total_trades} trades)\n"
                    f"📊 Active: `{len(active_trades)}/{MAX_ACTIVE_TRADES}`"
                )
                
                # Always send signal to Telegram
                safe_telegram_send(signal_text)
                logger.info(f"📨 Signal Sent for {symbol} to Telegram.")

                # ============================================================
                # EXECUTE TRADE ONLY IF SLOT AVAILABLE
                # ============================================================
                if len(active_trades) >= MAX_ACTIVE_TRADES:
                    logger.info(f"⏸️ Max trades hit ({MAX_ACTIVE_TRADES}), but Signal Sent to TG for {symbol}.")
                    continue # Skip execution, but signal was already sent

                if not win_rate_ok:
                    logger.info(f"⏭️ SKIPPED EXECUTION {symbol}: Global Win rate too low.")
                    continue
                ...
                if success:
                    ...
                    logger.info(f"✅ {mode} order: {symbol} {side} @ {current_price:.2f} (Size: {size:.4f} = 30% of portfolio)")
...
def generate_chart(symbol, df, patterns=None):
    try:
        ...
        if len(df) >= 20:
            ...
            plt.plot(dates, sma_20.values, label='SMA 20', color='orange', linestyle='--', alpha=0.7)
        
        if len(df) >= 50:
            sma_50 = df['close'].rolling(50).mean()
            plt.plot(dates, sma_50.values, label='SMA 50', color='red', linestyle='--', alpha=0.7)
...
def get_stats():
    ...
    stats['max_active_trades'] = MAX_ACTIVE_TRADES
...
logger.info("✅ Starting Quantum Whale Terminal v8.1...")
logger.info(f"📊 Mode: {TRADING_MODE}")
...
logger.info(f"🔄 Auto-Sync: ENABLED (updates every 25s)")
