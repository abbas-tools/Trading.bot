...
# If eventlet not available, fallback to threading
try:
    import eventlet
...
if __name__ == '__main__':
    ...
    # Use eventlet if available, otherwise fallback to werkzeug
    try:
        import eventlet
        eventlet.monkey_patch()
        logger.info(f"🚀 Starting with eventlet on port {port}")
        socketio.run(app, host='0.0.0.0', port=port, debug=False)
    except ImportError:
        logger.warning("⚠️ eventlet not available, using werkzeug")
