...
# ============================================================
# ENHANCED POSITION SIZING - FIXED MINIMUM LOT SIZE
# ============================================================

# Position sizing configuration
MIN_LOT_SIZE = 0.001  # Minimum position size (NEVER goes below this!)
...
def calculate_smart_position_size(capital, kelly_fraction, atr, current_price, confidence_score=1.0):
    """Enhanced position sizing with minimum lot enforcement"""
    try:
        ...
        base_position = capital * kelly_fraction / current_price
        
        # Apply confidence adjustment
        adjusted_position = base_position * confidence_score
        
        # Risk-based position (2% of capital)
        risk_position = (capital * MAX_RISK_PER_TRADE) / (atr / current_price * 1.5) if atr > 0 else 0
        
        # Take smaller of Kelly and Risk-based
        final_position = min(adjusted_position, risk_position)
        ...
        # CRITICAL: Enforce minimum lot size
...
# ============================================================
# ENHANCED INTELLIGENCE: SMART TRADE SCORING
# ============================================================

class TradeIntelligence:
    """Enhanced trade intelligence with multi-factor scoring"""
    ...
    def calculate_trade_score(self, df, side, current_price):
        """Calculate comprehensive trade score (0-100)"""
        try:
            scores = []
            weights = {
                'trend': 0.25,
            ...
            logger.debug(f"📊 Trade Scores: {', '.join([f'{name}={val:.2f}' for name, val in scores])}")
            return confidence
...
logger.info(f"🧠 Smart Intelligence: ENABLED, Min Position: {MIN_LOT_SIZE}")
...
def init_db():
    with db_manager.get_connection() as conn:
        ...
        ''')
        conn.commit()
        logger.info("✅ Database initialized with confidence tracking")

init_db()

# --- DATABASE FUNCTIONS ---
def log_to_db(symbol, side, entry, size, pnl, status, order_id=None, mode='PAPER', exchange='bitget', chart_url=None, kelly_used=None, atr_used=None, confidence_score=None):
    try:
        with db_manager.get_connection() as conn:
            cursor = conn.cursor()
            timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
            cursor.execute(
                "INSERT INTO trades (timestamp, symbol, side, entry, size, pnl, status, order_id, mode, exchange, chart_url, kelly_used, atr_used, confidence_score) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (timestamp, symbol, side, entry, size, pnl, status, order_id, mode, exchange, chart_url, kelly_used, atr_used, confidence_score)
            )
            conn.commit()
    except Exception as e:
...
def get_statistics():
    try:
        with db_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) as total, SUM(CASE WHEN status='WIN' THEN 1 ELSE 0 END) as wins, SUM(pnl) as total_pnl, AVG(confidence_score) as avg_confidence FROM trades")
            row = cursor.fetchone()
            if row:
                ...
                return {"total": total, "wins": wins, "win_rate": win_rate, "total_pnl": total_pnl, "avg_confidence": avg_confidence}
    except Exception as e:
        logger.error(f"❌ Statistics error: {e}")
    return {"total": 0, "wins": 0, "win_rate": 0, "total_pnl": 0, "avg_confidence": 0}
...
# ============================================================
# REST OF THE FUNCTIONS (Keep all existing functions)
# ============================================================
        ...
        try:
            ...
            if side == "BUY":
                if current_price >= tp:
                    ...
                    log_to_db(symbol, side, entry, size, profit, "WIN", order_id, mode, exchange, None, atr, None)
                    ...
                    log_to_db(symbol, side, entry, size, -loss, "LOSS", order_id, mode, exchange, None, atr, None)
                    del active_trades[symbol]
                    del position_data[symbol]
...
# ============================================================
# MODIFIED analyze_and_trade() WITH SMART SCORING & MIN LOT
# ============================================================
...
def analyze_and_trade():
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            confidence_factor = confidence_score / 100  # 0.0 to 1.0
            ...
            # Use enhanced position sizing with minimum lot enforcement
            size = calculate_smart_position_size(current_capital, kelly_fraction, atr, current_price, confidence_factor)
            ...
            if success:
                ...
                log_to_db(symbol, side, current_price, size, 0, "OPEN", order_id, mode, exchange, None, kelly_fraction, atr, confidence_score)
                logger.info(f"✅ {mode} order: {symbol} {side} @ {current_price:.2f} (Confidence: {confidence_score:.0f}%, Kelly: {kelly_fraction:.2%})")
    ...
    while not _shutting_down:
        try:
            ...
            socketio.emit('market_update', {
                ...
                'avg_confidence': get_statistics().get('avg_confidence', 0),
                'market_sentiment': {
