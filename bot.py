...
MAX_RISK_PER_TRADE = 0.02  # Reduced from 0.03
RISK_REWARD_RATIO = 2.0    # Reduced from 3.0 for better win rate
MAX_LEVERAGE = 5           # REDUCED from 20 to 5 for safety
MIN_LEVERAGE = 1
DEFAULT_LEVERAGE = 3       # REDUCED from 10 to 3
...
def calculate_smart_position_size(capital, kelly_fraction, atr, current_price, confidence_score=1.0):
    try:
        ...
        # Risk-based sizing with wider SL
...
MIN_WIN_RATE = 55.0      # Reduced from 65% for realistic start
MIN_TRADES_FOR_WIN_RATE = 5
MIN_ATR_PERCENT = 0.3    # Slightly wider
MAX_ATR_PERCENT = 4.0    # Slightly wider
MIN_VOLUME_MULTIPLIER = 1.0  # Reduced from 1.1
RSI_OVERBOUGHT = 75
RSI_OVERSOLD = 25
...
MIN_Kelly_FRACTION = 0.02  # Reduced
MAX_Kelly_FRACTION = 0.12  # Reduced

MTF_TIMEFRAMES = ["15m", "1H"]
REQUIRE_ALL_MTF_ALIGN = False  # Changed to False for more signals

ATR_SL_MULTIPLIER_BASE = 2.5  # INCREASED from 1.5 to 2.5 (wider SL)
ATR_TP_MULTIPLIER_BASE = 4.0  # INCREASED from 3.0 to 4.0
...
def calculate_dynamic_kelly():
    ...
    try:
        ...
        if total < 5:
            base_kelly = 0.08  # Conservative start
        else:
...
def check_strict_mtf_confirmation(symbol, side):
    ...
    try:
        ...
        if side == "BUY":
            ...
            elif bullish_count < total_mtf * 0.4:  # Relaxed from 0.5
...
# ============================================================
# MANAGE TRADES - IMPROVED PNL CALCULATION
# ============================================================
...
def manage_trades():
    ...
    for symbol in list(active_trades.keys()):
        try:
            ...
            # ============================================================
            # IMPROVED PNL CALCULATION - CRYPTO FUTURES STANDARD
            # ============================================================
            if side == "BUY":
                ...
                # PnL = (Entry Price - Current Price) * (Position Size / Entry Price)
            ...
            # Trailing stop loss - wider for better win rate
            if side == "BUY" and (current_price - entry) > (atr * 1.5):
                new_sl = current_price - (atr * 2.0)  # Wider trailing
                if new_sl > sl:
                    trade['sl'] = sl = new_sl
            ...
            # Check TP and SL with proper PnL calculation
            if side == "BUY":
                if current_price >= tp:
    ...
    try:
        ...
        for symbol in trading_symbols[:10]:  # Top 10 coins
            ...
            try:
                ...
                # TRADE ON COINS WITH SCORE > 0.4 (relaxed from 0.5)
                if score_value < 0.4:
                ...
                # Calculate leverage (dynamic based on confidence - capped at MAX_LEVERAGE)
