# ============================================================
# ULTIMATE QUANTUM WHALE v18.4 - COMPLETE FIXED VERSION
# FIXED: Strategies loading in consensus
# FIXED: Signal generation for trading
# ADDED: 5-Minute Timeframe Integration
# OPTIMIZED: Trade execution on 5m & 15m only
# ============================================================
import eventlet
...
# ✅ UPDATED: 4 TIMEFRAMES - 5m ADDED
MTF_TIMEFRAMES = ["5m", "15m", "1H", "4H"]  # 5m ADDED
TRADE_TIMEFRAMES = ["5m", "15m"]  # ONLY THESE FOR TRADE EXECUTION
ANALYSIS_TIMEFRAMES = ["5m", "15m", "1H", "4H"]  # ALL FOR ANALYSIS
...
def init_db():
    with db_manager.get_connection() as conn:
        ...
        conn.commit()
    logger.info("✅ Database initialized with v18.4 Ultimate features")
...
# ============================================================
# CANDLE FETCHER - UPDATED WITH MULTIPLE TIMEFRAMES
# ============================================================
    ...
    try:
        ...
        if sr_type == 'NEAR_SUPPORT':
            if pattern in ["BEARISH_ENGULFING", "SHOOTING_STAR"]:
                ...
                result['confidence'] = 0
                result['reason'] = '🚫 BLOCKED: SELL at SUPPORT'
            ...
            if pattern in ["BULLISH_ENGULFING", "HAMMER"]:
                result['signal'] = 'NEUTRAL'
                result['confidence'] = 0
                result['reason'] = '🚫 BLOCKED: BUY at RESISTANCE'
...
# ============================================================
# ULTIMATE TREND ANALYZER - UPDATED WITH 5m
# ============================================================
class UltimateTrendAnalyzer:
    ...
    def analyze_trend(self, symbol: str) -> Dict:
        try:
            timeframes = MTF_TIMEFRAMES  # Now includes 5m, 15m, 1H, 4H
            trends = {}
            details = {}
...
class AdaptiveLearner:
    ...
    def log_learning_insight(self, symbol: str, action: str, outcome: str, 
        ...
        try:
            with db_manager.get_connection() as conn:
                conn.execute("""
                    INSERT INTO learning_logs (timestamp, symbol, action, outcome, confidence, sentiment, regime, pnl, weekly_trend, daily_trend, macro_bias, timeframe)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (time.strftime('%Y-%m-%d %H:%M:%S'), symbol, action, outcome, confidence, sentiment, regime, pnl, weekly_trend, daily_trend, macro_bias, timeframe))
                conn.commit()
...
# ============================================================
# CONSENSUS ENGINE - 7 STRATEGIES - FIXED
# ============================================================
...
# ============================================================
# MULTI-TIMEFRAME ANALYZER - UPDATED
# ============================================================
class MultiTimeframeAnalyzer:
    def __init__(self):
        self.timeframes = MTF_TIMEFRAMES  # 5m, 15m, 1H, 4H
        self.trend_analyzer = UltimateTrendAnalyzer()
...
"""
    ...
    def analyze_and_trade(self):
        ...
        # ✅ TRADE EXECUTION ON 5m AND 15m TIMEFRAMES ONLY
        ...
        for symbol, score in top_coins[:10]:
            for tf in trade_timeframes:
                try:
                    ...
                    logger.info(f"📤 SIGNAL: {symbol} {consensus['signal']} @ {current_price:.4f} ({tf}) {'BREAKOUT!' if is_breakout and breakout_confirmed else ''}")
                    ...
                    if len(self.active_trades) < MAX_ACTIVE_TRADES:
                        ...
                        if success:
                            ...
                            logger.info(f"✅ EXECUTED: {symbol} {consensus['signal']} @ {current_price:.4f} ({tf})")
                except Exception as e:
        ...
        for symbol in list(self.active_trades.keys()):
            try:
                ...
                if side == "BUY" and current_price >= tp:
                    ...
                    self.adaptive_learner.log_learning_insight(symbol, side, "WIN", trade.get('confidence', 0), sentiment, regime, profit, weekly_trend, "NEUTRAL", macro_bias, timeframe)
                    safe_telegram_send(f"🎯 TP HIT {symbol}: +${profit:.4f} | Capital: ${self.current_capital:.2f} ({timeframe})")
                    self.log_trade(symbol, side, entry, size, profit, "WIN", trade.get('order_id'), trade.get('mode'), trade.get('confidence', 0), "", pattern, "TP Hit", rr_ratio, "", "", 0, trade.get('pattern_weight', 1.0), trade.get('var_status', 'LOW'), 0, regime, self.daily_pnl, 0, weekly_trend, 0, "", 0, "", 0, timeframe)
                    ...
                    self.adaptive_learner.log_learning_insight(symbol, side, "LOSS", trade.get('confidence', 0), sentiment, regime, -loss, weekly_trend, "NEUTRAL", macro_bias, timeframe)
                    safe_telegram_send(f"🛑 SL HIT {symbol}: -${loss:.4f} | Capital: ${self.current_capital:.2f} ({timeframe})")
                    self.log_trade(symbol, side, entry, size, -loss, "LOSS", trade.get('order_id'), trade.get('mode'), trade.get('confidence', 0), "", pattern, "SL Hit", rr_ratio, "", "", 0, trade.get('pattern_weight', 1.0), trade.get('var_status', 'LOW'), 0, regime, self.daily_pnl, 0, weekly_trend, 0, "", 0, "", 0, timeframe)
                    del self.active_trades[symbol]
                    del self.position_data[symbol]
    ...
    def run(self):
        ...
        logger.info("🐋 Quantum Whale v18.4 ULTIMATE - WORLD'S #1 TRADER")
        logger.info("=" * 70)
        ...
        logger.info("  ✅ 4 Timeframes (5m, 15m, 1H, 4H) - 5m ADDED!")
        ...
        logger.info("=" * 70)
        logger.info(f"📊 Strategies: 7")
        logger.info(f"🎯 Consensus Threshold: {CONSENSUS_THRESHOLD}/7")
...
def index():
    return render_template('index.html') if os.path.exists('templates/index.html') else jsonify({
        "status": "online",
        "bot": "Ultimate Quantum Whale v18.4",
        "mode": TRADING_MODE,
...
if __name__ == '__main__':
    logger.info("🚀 Starting Quantum Whale v18.4 ULTIMATE - WORLD'S #1 TRADER")
    logger.info("📊 ALL features integrated - 100% COMPLETE!")
    ...
    logger.info("📈 4 Timeframes (5m, 15m, 1H, 4H) - 5m ADDED!")
    logger.info("⏱️ TRADE EXECUTION: 5m & 15m ONLY")
    logger.info("🧠 Weekly Trend Analysis ENABLED")
    logger.info("🎯 Optimized R:R: 1.5-3.0")
    ...
    logger.info("🔗 Full Bitget API Integration ENABLED")
    logger.info("=" * 70)
