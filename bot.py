...
# FIXED: No SELL at Support, No BUY at Resistance
# ENHANCED: Top 50 Coins + Gold + Breakout Detection
# INTEGRATED: UptimeRobot Ping Handler - NO SLEEP MODE
# NEW: Macroeconomic Sentiment & News Filter
# NEW: NY Golden Hour Strategy (7th Strategy)
# NEW: Consensus Threshold 4/7
# FIXED: MacroIndicator AttributeError (NoneType)
# FIXED: /macro endpoint error handling
# FIXED: WebSocket reconnection stability
# ============================================================
import eventlet
...
# ============================================================
# CONFIGURATION - ULTIMATE v18 WITH ALL FIXES
# ============================================================
...
# ============================================================
# v18 TRADING CONSTANTS - ENHANCED FOR SAFE TRADING
# ============================================================
MAX_ACTIVE_TRADES = 5
...
CONSENSUS_THRESHOLD = 4  # UPDATED: 4/7 strategies (was 3/6)
...
# ============================================================
# MACROECONOMIC INDICATOR CONSTANTS - NEW
# ============================================================
MACRO_UPDATE_INTERVAL = 300  # 5 minutes
HIGH_IMPACT_NEWS_PAUSE_DURATION = 3600  # 1 hour before/after
...
def init_db():
    with db_manager.get_connection() as conn:
        ...
        conn.commit()
    logger.info("✅ Database initialized with v18 Ultimate features + Macro + Golden Hour")
...
class AsyncPriceFetcher:
    ...
    def start(self):
        ...
        self._thread.start()
        logger.info(f"🚀 Async price fetcher started (100ms updates) for {len(ALL_SYMBOLS)} symbols")
...
# ============================================================
# v18 INDICATOR FUNCTIONS - ALL PRESERVED
# ============================================================
...
def detect_breakout(df, lookback=BREAKOUT_LOOKBACK, volume_mult=BREAKOUT_VOLUME_MULTIPLIER):
    try:
        ...
        if len(close) > BREAKOUT_CONFIRMATION_CANDLES + 1:
            confirmation_candles = 0
            if resistance_breakout:
                for i in range(BREAKOUT_CONFIRMATION_CANDLES):
                    if close[-1-i] > prev_high:
                        confirmation_candles += 1
            elif support_breakout:
                for i in range(BREAKOUT_CONFIRMATION_CANDLES):
                    if close[-1-i] < prev_low:
                        confirmation_candles += 1
        
        confirmed = (confirmation_candles >= BREAKOUT_CONFIRMATION_CANDLES) if 'confirmation_candles' in locals() else False
...
# ============================================================
# TREND STRENGTH ANALYZER - ENHANCED
# ============================================================
...
# ============================================================
# FEAR & GREED / BTC TREND - PRESERVED
# ============================================================
...
# ============================================================
# v18 ULTIMATE MARKET REGIME DETECTION - PRESERVED
# ============================================================
...
# ============================================================
# v18 ULTIMATE TREND ANALYZER - PRESERVED
# ============================================================
...
class AdaptiveLearner:
    ...
    def log_learning_insight(self, symbol: str, action: str, outcome: str, 
        ...
        try:
            with db_manager.get_connection() as conn:
                conn.execute("""
                    INSERT INTO learning_logs (timestamp, symbol, action, outcome, confidence, sentiment, regime, pnl, weekly_trend, daily_trend, macro_bias)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (time.strftime('%Y-%m-%d %H:%M:%S'), symbol, action, outcome, confidence, sentiment, regime, pnl, weekly_trend, daily_trend, macro_bias))
                conn.commit()
...
# ============================================================
# v18 PATTERN DETECTION - ENHANCED WITH SR CONTEXT
# ============================================================
...
# ============================================================
# ENHANCED PATTERN RECOGNITION WITH SR CONTEXT
# ============================================================
    ...
    try:
        ...
        if sr_type == 'NEAR_SUPPORT':
            if pattern in ["BEARISH_ENGULFING", "SHOOTING_STAR"]:
                ...
                result['confidence'] = 0
                result['reason'] = '🚫 BLOCKED: SELL at SUPPORT - Wait for breakout'
            elif pattern in ["BULLISH_ENGULFING", "HAMMER"]:
                result['signal'] = 'BUY'
                result['confidence'] = 75
                result['reason'] = 'Support bounce with bullish pattern'
            else:
                result['signal'] = 'BUY'
                result['confidence'] = 60
                result['reason'] = 'Price near support - Look for bounce'
                
        elif sr_type == 'NEAR_RESISTANCE':
            if pattern in ["BULLISH_ENGULFING", "HAMMER"]:
                result['signal'] = 'NEUTRAL'
                result['confidence'] = 0
                result['reason'] = '🚫 BLOCKED: BUY at RESISTANCE - Wait for breakout'
            elif pattern in ["BEARISH_ENGULFING", "SHOOTING_STAR"]:
                result['signal'] = 'SELL'
                result['confidence'] = 75
                result['reason'] = 'Resistance rejection with bearish pattern'
            else:
                result['signal'] = 'SELL'
                result['confidence'] = 60
                result['reason'] = 'Price near resistance - Look for rejection'
                
        else:
            if pattern in ["BULLISH_ENGULFING", "HAMMER"]:
                result['signal'] = 'BUY'
                result['confidence'] = 70
                result['reason'] = 'Bullish pattern detected'
            elif pattern in ["BEARISH_ENGULFING", "SHOOTING_STAR"]:
                result['signal'] = 'SELL'
                result['confidence'] = 70
                result['reason'] = 'Bearish pattern detected'
                
        return result
    except Exception as e:
        logger.debug(f"Pattern with SR error: {e}")
        return {'pattern': 'NONE', 'signal': 'NEUTRAL', 'confidence': 0, 'reason': 'Error'}

# ============================================================
# v18 STRATEGIES (6 STRATEGIES) - ENHANCED
# ============================================================
...
# ============================================================
# FIXED: MACROECONOMIC SENTIMENT & NEWS FILTER MODULE
# ============================================================
class MacroIndicator:
    """Fetches and manages macroeconomic data for market sentiment"""
    
    def __init__(self):
        # FIXED: Changed dxy from None to empty dict
        self.current_data = {
            'nfp': None,
            'cpi': None,
            'fomc': None,
            'unemployment': None,
            'gdp': None,
            'dxy': {},  # FIXED: Empty dict instead of None
            'sentiment': 'NEUTRAL',
            'last_update': 0,
            'high_impact_events': [],
        ...
        self._running = False
        self._thread = None
        self._dxy_history = []  # FIXED: Initialize history here
    ...
    def _fetch_macro_data(self):
        try:
            ...
            with self.lock:
                # FIXED: Always ensure dxy is a dict
                if dxy_data:
    ...
    def _fetch_dxy(self) -> Optional[Dict]:
        try:
            url = "https://api.bitget.com/api/v2/mix/market/ticker?symbol=DXYUSDT&productType=usdt-futures"
            response = requests.get(url, timeout=5)
            data = response.json()
            if data.get('code') == '00000':
                ticker = data.get('data', {})
    ...
    def _update_dxy_trend(self, current_price: float):
        """Update DXY trend based on recent price action"""
        if not hasattr(self, '_dxy_history'):
            self._dxy_history = []
    ...
    def _check_high_impact_events(self):
        ...
        # Simulated high-impact events - In production, use a proper economic calendar API
        events = []
        
        with self.lock:
            self.current_data['high_impact_events'] = events
            
            is_high_impact_near = False
            for event in events:
                if event.get('impact') == 'HIGH':
                    event_time = event.get('timestamp')
                    if event_time:
                        time_diff = abs((event_time - now).total_seconds())
                        if time_diff < HIGH_IMPACT_NEWS_PAUSE_DURATION:
                            is_high_impact_near = True
                            self.current_data['pause_reason'] = f"High-impact {event.get('name')} in {time_diff/60:.0f} min"
                            break
                            
            self.current_data['is_trading_paused'] = is_high_impact_near
            if is_high_impact_near:
                logger.warning(f"⚠️ High-impact news: {self.current_data['pause_reason']}")
...
# ============================================================
# NEW: NEW YORK SESSION "GOLDEN HOUR" STRATEGY (7TH STRATEGY)
# ============================================================
class GoldenHourStrategy:
    """
    New York Session Golden Hour Strategy
    Time: 4:30 PM IST - 5:30 PM IST: Range Building
    Time: 5:30 PM IST - 6:30 PM IST: Execution Window
    
    Detects liquidity sweeps and high-probability reversal entries
    """
    ...
    def __init__(self):
        ...
        # Time windows in IST (India Standard Time)
        self.RANGE_BUILD_START = 16.5  # 4:30 PM
        self.RANGE_BUILD_END = 17.5    # 5:30 PM
        self.EXECUTION_START = 17.5    # 5:30 PM
        self.EXECUTION_END = 18.5      # 6:30 PM
...
# ============================================================
# v18 ULTIMATE CONSENSUS ENGINE - UPDATED WITH 7 STRATEGIES
# ============================================================
class ConsensusEngine:
    def __init__(self):
        self.strategies = [
            ...
            GoldenHourStrategy()  # NEW: 7th Strategy
        ]
        self.consensus_threshold = CONSENSUS_THRESHOLD  # 4/7
    ...
    def analyze(self, df: pd.DataFrame, symbol: str, positions: Dict) -> Dict:
        ...
        # FIXED: Ensure macro_data is always a dict
        ...
        # Golden Hour boost
        golden_hour_strategy = None
        for s in results:
            if s['name'] == 'NY Golden Hour' and s['signal'] != 'NEUTRAL':
                golden_hour_strategy = s
                break
                
        if golden_hour_strategy and golden_hour_strategy['confidence'] >= 70:
            if golden_hour_strategy['signal'] == 'BUY':
                buy_confidence = min(95, buy_confidence * 1.2)
                buy_count += 1
            elif golden_hour_strategy['signal'] == 'SELL':
                sell_confidence = min(95, sell_confidence * 1.2)
                sell_count += 1
        ...
        # CONSENSUS THRESHOLD: 4/7
        if buy_count >= self.consensus_threshold:
            final_signal = 'BUY'
            final_confidence = min(95, buy_confidence + 10)
            if is_breakout and breakout_confirmed and breakout_type == 'RESISTANCE_BREAKOUT':
                final_confidence = min(98, final_confidence + 10)
                reason = f"✅ BREAKOUT! {buy_count}/7 strategies agree on BUY"
            elif weekly_trend in ['STRONG_BULLISH', 'BULLISH']:
                final_confidence = min(95, final_confidence * 1.05)
                reason = f"{buy_count}/7 strategies agree on BUY (Weekly: {weekly_trend})"
            else:
                reason = f"{buy_count}/7 strategies agree on BUY (Regime: {current_regime})"
            
        elif sell_count >= self.consensus_threshold:
            final_signal = 'SELL'
            final_confidence = min(95, sell_confidence + 10)
            if is_breakout and breakout_confirmed and breakout_type == 'SUPPORT_BREAKOUT':
                final_confidence = min(98, final_confidence + 10)
                reason = f"✅ BREAKOUT! {sell_count}/7 strategies agree on SELL"
            elif weekly_trend in ['STRONG_BEARISH', 'BEARISH']:
                final_confidence = min(95, final_confidence * 1.05)
                reason = f"{sell_count}/7 strategies agree on SELL (Weekly: {weekly_trend})"
            else:
                reason = f"{sell_count}/7 strategies agree on SELL (Regime: {current_regime})"
        else:
...
# ============================================================
# v18 ULTIMATE MULTI-TIMEFRAME ANALYZER - PRESERVED
# ============================================================
...
# ============================================================
# v18 ULTIMATE RISK MANAGER - PRESERVED
# ============================================================
class RiskManager:
    def __init__(self):
...
class CircuitBreaker:
    ...
    def check_circuit_breaker(self, current_capital: float) -> Tuple[bool, str]:
        ...
        if self.daily_losses >= 4 and self.daily_wins == 0:
            self.is_breached = True
            self.breach_reason = "4 consecutive losses without a win"
            return False, self.breach_reason
        return True, "OK"
    ...
    def predict(self, df: pd.DataFrame) -> Dict:
        try:
            ...
            return {
                'signal': signal,
                'confidence': min(95, confidence),
                'details': f"ML: {len([s for s in scores if s[1] != 'NEUTRAL'])} active indicators"
            }
        except Exception as e:
...
# ============================================================
# TOP MOVERS TRACKER - ENHANCED (24h Gainers & Losers)
# ============================================================
class TopMoversTracker:
    ...
    def update(self):
        try:
            ...
            self.top_gainers = sorted(movers, key=lambda x: x['change_24h'], reverse=True)[:TOP_MOVERS_COUNT]
            self.top_losers = sorted(movers, key=lambda x: x['change_24h'])[:TOP_MOVERS_COUNT]
            ...
            try:
                ...
                logger.debug(f"Top movers DB error: {e}")
    ...
    def get_best_opportunities(self, count=5):
        ...
        for g in self.top_gainers[:10]:
            if g['volume_ratio'] > 1.5 and g['atr_pct'] > 1.0:
                if g['is_breakout'] and g['breakout_type'] == 'RESISTANCE_BREAKOUT':
                    opportunities.append({
                        'symbol': g['symbol'],
                        'score': g['momentum_score'] * 1.5,
                        'type': 'BREAKOUT_GAINER',
                        'change': g['change_24h'],
                        'sr_type': g['sr_type']
                    })
                else:
        ...
        for l in self.top_losers[:10]:
            if l['volume_ratio'] > 2.0 and l['atr_pct'] > 1.5:
                if l['is_breakout'] and l['breakout_type'] == 'SUPPORT_BREAKOUT':
                    opportunities.append({
                        'symbol': l['symbol'],
                        'score': -l['momentum_score'] * 1.2,
                        'type': 'BREAKOUT_LOSER',
                        'change': l['change_24h'],
                        'sr_type': l['sr_type']
                    })
    ...
    def get_mover_data(self, symbol: str):
        return self.all_movers.get(symbol)
...
class QuantumWhaleBot:
    ...
    def generate_signal_message(self, symbol: str, side: str, confidence: float,
                               ...
                               macro_bias: str = "NEUTRAL", golden_hour: bool = False) -> str:
        ...
        breakout_note = ""
        if is_breakout:
            breakout_note = f"🔓 *{breakout_type} CONFIRMED!*"
...
🧠 *STRATEGY CONSENSUS (7 STRATEGIES):*
{strategy_details}
...
"""
    ...
    def analyze_and_trade(self):
        ...
        if self.circuit_breaker.daily_losses >= 4 and self.circuit_breaker.daily_wins == 0:
            return
        ...
        for symbol, score in top_coins[:10]:
            try:
                ...
                signal_text = self.generate_signal_message(
                    ...
                    macro_bias, golden_hour_active
                )
                
                safe_telegram_send(signal_text)
                logger.info(f"📤 SIGNAL: {symbol} {consensus['signal']} @ {current_price:.4f} {'BREAKOUT!' if is_breakout and breakout_confirmed else ''}")
        ...
        for symbol in list(self.active_trades.keys()):
            try:
                ...
                if side == "BUY" and current_price >= tp:
                    ...
                    self.adaptive_learner.log_learning_insight(symbol, side, "WIN", trade.get('confidence', 0), sentiment, regime, profit, weekly_trend, "NEUTRAL", macro_bias)
                    ...
                    self.adaptive_learner.log_learning_insight(symbol, side, "LOSS", trade.get('confidence', 0), sentiment, regime, -loss, weekly_trend, "NEUTRAL", macro_bias)
    ...
    def log_trade(self, symbol, side, entry, size, pnl, status, order_id, mode,
                  ...
                  macro_bias="NEUTRAL", golden_hour=0):
        try:
            with db_manager.get_connection() as conn:
                timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
                conn.execute("""
    ...
    def run(self):
        ...
        logger.info("🐋 Quantum Whale v18.0 ULTIMATE - WORLD'S #1 TRADER")
        logger.info("=" * 70)
        logger.info("📊 ULTIMATE Features:")
        logger.info("  ✅ 7 Strategy Ensemble (NEW: NY Golden Hour Strategy)")
        ...
        logger.info("  ✅ 3 Timeframes (15m, 1H, 4H)")
        ...
        logger.info("=" * 70)
        logger.info(f"📊 Strategies: 7 (was 6)")
        logger.info(f"🎯 Consensus Threshold: {CONSENSUS_THRESHOLD}/7 (was 3/6)")
        ...
        logger.info("📡 Macro Indicators: DXY, NFP, CPI, FOMC, Unemployment")
        logger.info("⏰ NY Golden Hour: 4:30-6:30 PM IST")
        logger.info("=" * 70)
...
def index():
    return render_template('index.html') if os.path.exists('templates/index.html') else jsonify({
        "status": "online",
        "bot": "Ultimate Quantum Whale v18.0",
        "mode": TRADING_MODE,
...
if __name__ == '__main__':
    logger.info("🚀 Starting Quantum Whale v18.0 ULTIMATE - WORLD'S #1 TRADER")
    logger.info("📊 ALL features integrated - 100% PRESERVED!")
    logger.info("📡 Real-time updates every 100ms")
    logger.info("⚡ MAX 5 Concurrent Trades")
    logger.info("📊 7 Strategies Ensemble (NEW: NY Golden Hour)")
    logger.info("📈 3 Timeframes (15m, 1H, 4H)")
    logger.info("🧠 Weekly Trend Analysis ENABLED")
    logger.info("🎯 Optimized R:R: 1.5-3.0 for easier TP hits")
    logger.info("⚡ Consensus Threshold: 4/7 strategies")
    logger.info("🎯 Min Confidence: 60%")
    logger.info("🚀 Trailing Stop: Activation 1.0 ATR, Step 0.2 ATR")
    logger.info("🛡️ NO SELL at SUPPORT | NO BUY at RESISTANCE")
    logger.info(f"📊 Total Symbols: {len(ALL_SYMBOLS)} (including Gold)")
    logger.info("📊 24h Top Gainers & Losers Tracking ENABLED")
    logger.info("🔓 Breakout Detection & Confirmation ENABLED")
    logger.info("📡 Macroeconomic Sentiment & News Filter ENABLED")
    logger.info("⏰ NY Golden Hour Strategy ENABLED (4:30-6:30 PM IST)")
    logger.info("🔄 Liquidity Sweep Detection ENABLED")
    logger.info("=" * 70)
