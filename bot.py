...
# ============================================================
# ASYNC HELPER FOR EXTERNAL APIS (v19.0) - WITH RETRY LOGIC
# ============================================================
async def async_fetch(session, url, params=None, headers=None, timeout=10, retries=3):
        ...
        try:
            async with session.get(url, params=params, headers=headers, timeout=timeout) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Async fetch attempt {attempt+1}/{retries} failed: {response.status}")
        except Exception as e:
            logger.warning(f"Async fetch attempt {attempt+1}/{retries} error: {e}")
            if attempt < retries - 1:
                await asyncio.sleep(2 ** attempt)  # exponential backoff
    return None

def run_async_fetch(url, params=None, headers=None):
    """Synchronous wrapper to run async fetch with retries"""
...
# ============================================================
# CANDLE FETCHER - FIXED: USING SYNC BITGET REQUEST WITH RETRY
# ============================================================
def fetch_candles(symbol, granularity="1H", limit=100):
    ...
    # ✅ Use synchronous bitget_public_request with retry (more stable with eventlet)
    ...
    for attempt in range(3):
        try:
            ...
            if response and response.get('code') == '00000':
                ...
                logger.warning(f"Candle fetch attempt {attempt+1} failed for {symbol} {granularity}")
        except Exception as e:
            logger.warning(f"Candle fetch attempt {attempt+1} error: {e}")
