...
# ============================================================
# TRADE FILTERS - Minimum Win Rate Threshold
# ============================================================

MIN_WIN_RATE = 80.0  # Minimum win rate % to execute trades (80% = 0.80)
MIN_TRADES_FOR_WIN_RATE = 10  # Minimum number of trades before using win rate filter
...
def calculate_kelly():
    try:
        ...
        return max(0.03, min(kelly * 0.3, 0.15))
    except Exception:
        return 0.10
...
def calculate_position_size(capital, kelly, atr, price):
    ...
    if capital <= 0:
        capital = 1.0
    base = capital * kelly / price
    max_size = min(MAX_POSITION_SIZE, capital * 0.02 / price)
    return max(MIN_POSITION_SIZE, min(base, max_size))

def execute_order(symbol, side, size, exchange=PRIMARY_EXCHANGE):
    global TRADING_MODE

    # Check if we have valid API keys for DEMO/REAL
...
def analyze_and_trade():
    ...
    # Get statistics once for all symbols
    stats = get_statistics()
    ...
    if total_trades >= MIN_TRADES_FOR_WIN_RATE:
        if win_rate < MIN_WIN_RATE:
            win_rate_ok = False
            win_rate_message = f"⏭️ Win Rate {win_rate:.1f}% < {MIN_WIN_RATE}% (Trades: {total_trades})"
            logger.info(f"⏭️ SKIPPING ALL TRADES: {win_rate_message}")
        else:
            win_rate_message = f"✅ Win Rate {win_rate:.1f}% >= {MIN_WIN_RATE}% (Trades: {total_trades})"
            logger.info(f"✅ WIN RATE OK: {win_rate_message}")
    else:
        win_rate_message = f"📊 Not enough trades: {total_trades}/{MIN_TRADES_FOR_WIN_RATE}"
        logger.info(f"📊 {win_rate_message} - Trading with signals")

    for symbol in SYMBOLS:
        if symbol in active_trades:
            continue

        # Skip if win rate is below threshold
        if not win_rate_ok:
            logger.info(f"⏭️ SKIPPED {symbol}: Low win rate {win_rate:.1f}% < {MIN_WIN_RATE}%")
            continue
        ...
        try:
            ...
            size = calculate_position_size(current_capital, kelly, atr, current_price)
            ...
            if success:
                ...
                logger.info(f"✅ {mode} order: {symbol} {side} @ {current_price:.2f} (WinRate: {win_rate:.1f}%)")
