...
MTF_CONFIRMATION_THRESHOLD = 0.50  # ⬆️ Increased for better confirmation
...
CONSENSUS_THRESHOLD = 3      # ⬆️ Increased for better confirmation
MIN_CONFIDENCE_SCORE = 60    # ⬆️ Increased for quality
SIGNAL_COOLDOWN = 60
...
MIN_LOT_SIZE = 0.001
LOT_SIZE_STEP = 0.001
MIN_ATR_PERCENT = 0.5        # ⬆️ Increased for quality setups
MAX_ATR_PERCENT = 10.0
MIN_VOLUME_MULTIPLIER = 0.5  # ⬆️ Increased for volume confirmation
RSI_OVERSOLD = 30
RSI_OVERBOUGHT = 70
...
def init_db():
    with db_manager.get_connection() as conn:
        ...
        conn.commit()
    logger.info("✅ Database initialized with v18 Ultimate features + Top Movers")
...
class AsyncPriceFetcher:
    ...
    def _fetch_single_price(self, symbol):
        try:
            # Special handling for Gold (XAUUSDT)
            if symbol == "XAUUSDT":
...
def detect_breakout(df, lookback=BREAKOUT_LOOKBACK, volume_mult=BREAKOUT_VOLUME_MULTIPLIER):
    """Detect breakout from support/resistance with volume confirmation"""
    try:
        ...
        # Check confirmation (price stayed above/below)
...
def detect_pattern_with_sr_context(df):
    """Detect patterns with support/resistance context"""
    try:
        ...
        # 🚫 FIX: No SELL at Support, No BUY at Resistance
        if sr_type == 'NEAR_SUPPORT':
            if pattern in ["BEARISH_ENGULFING", "SHOOTING_STAR"]:
                result['signal'] = 'NEUTRAL'  # Block SELL at support
                result['confidence'] = 0
            ...
            if pattern in ["BULLISH_ENGULFING", "HAMMER"]:
                result['signal'] = 'NEUTRAL'  # Block BUY at resistance
                result['confidence'] = 0
            ...
            # Neutral area - look for breakouts or reversals
            if pattern in ["BULLISH_ENGULFING", "HAMMER"]:
                result['signal'] = 'BUY'
...
class MomentumStrategy:
    ...
    def analyze(self, df: pd.DataFrame) -> Dict:
        try:
            ...
            # 🚫 FIX: Check SR context before RSI signals
            sr_type = sr_data.get('current_sr', 'NEUTRAL')
            
            if rsi < RSI_OVERSOLD:
                rsi_signal = 'BUY'
                rsi_confidence = 65 + (RSI_OVERSOLD - rsi) * 1.2
                # Block BUY if at resistance
...
class VolatilityBreakoutStrategy:
    ...
    def analyze(self, df: pd.DataFrame) -> Dict:
        try:
            ...
            # 🚫 FIX: Only trade breakouts, not at support/resistance
...
class PatternRecognitionStrategy:
    ...
    def analyze(self, df: pd.DataFrame) -> Dict:
        try:
            ...
            # Use enhanced pattern detection with SR context
...
class MeanReversionStrategy:
    ...
    def analyze(self, df: pd.DataFrame) -> Dict:
        try:
            ...
            # 🚫 FIX: Only trade mean reversion with SR context
...
# ============================================================
# v18 ULTIMATE CONSENSUS ENGINE - ENHANCED
# ============================================================
class ConsensusEngine:
    ...
    def analyze(self, df: pd.DataFrame, symbol: str, positions: Dict) -> Dict:
        ...
        # 🚫 FIX: Boost breakout signals, block counter-trend at SR
        if is_breakout and breakout_confirmed:
            if breakout_type == 'RESISTANCE_BREAKOUT':
                buy_confidence *= 1.2
                buy_count += 1  # Virtual vote for breakout
            elif breakout_type == 'SUPPORT_BREAKOUT':
                sell_confidence *= 1.2
                sell_count += 1  # Virtual vote for breakout
        else:
            # Block trades at support/resistance without breakout
            if sr_type == 'NEAR_SUPPORT':
                sell_count = 0
                sell_confidence = 0
            elif sr_type == 'NEAR_RESISTANCE':
        ...
        if buy_count >= self.consensus_threshold:
            ...
            if is_breakout and breakout_confirmed and breakout_type == 'RESISTANCE_BREAKOUT':
                ...
                reason = f"✅ BREAKOUT! {buy_count}/6 strategies agree on BUY"
            elif weekly_trend in ['STRONG_BULLISH', 'BULLISH']:
                final_confidence = min(95, final_confidence * 1.05)
                reason = f"{buy_count}/6 strategies agree on BUY (Weekly: {weekly_trend})"
            else:
                reason = f"{buy_count}/6 strategies agree on BUY (Regime: {current_regime})"
            ...
            if is_breakout and breakout_confirmed and breakout_type == 'SUPPORT_BREAKOUT':
                final_confidence = min(98, final_confidence + 10)
                reason = f"✅ BREAKOUT! {sell_count}/6 strategies agree on SELL"
            elif weekly_trend in ['STRONG_BEARISH', 'BEARISH']:
                final_confidence = min(95, final_confidence * 1.05)
                reason = f"{sell_count}/6 strategies agree on SELL (Weekly: {weekly_trend})"
            else:
                reason = f"{sell_count}/6 strategies agree on SELL (Regime: {current_regime})"
        else:
            reason = f"No consensus ({buy_count} BUY, {sell_count} SELL, {neutral_count} NEUTRAL)"
            
        # 🚫 FINAL SAFETY CHECK: No SELL at support, No BUY at resistance
        ...
        # Only allow breakout trades if confirmed
...
class TopMoversTracker:
    ...
    def update(self):
        try:
            ...
            # Log top movers
            logger.info(f"📊 Top Gainers (24h): {[g['symbol'] + ' ' + str(round(g['change_24h'], 2)) + '%' for g in self.top_gainers[:5]]}")
            logger.info(f"📉 Top Losers (24h): {[l['symbol'] + ' ' + str(round(l['change_24h'], 2)) + '%' for l in self.top_losers[:5]]}")
    ...
    def get_best_opportunities(self, count=5):
        ...
        # Check gainers with breakout potential
        ...
        # Check losers for reversal potential
...
class QuantumWhaleBot:
    ...
    def generate_signal_message(self, symbol: str, side: str, confidence: float,
                               ...
                               sr_type: str, is_breakout: bool, breakout_type: str) -> str:
...
"""
    ...
    def _score_coins(self) -> Dict:
        ...
        for symbol in ALL_SYMBOLS:
            try:
                ...
                # Boost score for confirmed breakouts
                if is_breakout and breakout_confirmed:
                    score *= 1.3
                elif is_breakout:
                    score *= 1.1
                    
                # Reduce score at support/resistance without breakout
    ...
    def analyze_and_trade(self):
        ...
        # Update top movers (24h gainers & losers)
        ...
        for symbol, score in top_coins[:10]:
            try:
                ...
                # 🚫 FIX: Skip if at support/resistance without breakout
                ...
                # Check opposing signals - More lenient for breakouts
                has_strong_opposing = False
                if is_breakout and breakout_confirmed:
                    # Less strict for breakouts
                    for s in consensus['strategies']:
    ...
    def manage_trades(self):
        ...
        for symbol in list(self.active_trades.keys()):
            try:
                ...
                # Trailing Stop - More aggressive for better profit capture
            ...
            with db_manager.get_connection() as conn:
                ...
                conn.execute("""
                    ...
                    (timestamp, symbol, side, entry, size, pnl, status, order_id,
                     ...
                     is_breakout, sr_type, breakout_confirmed)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (timestamp, symbol, side, entry, size, pnl, status, order_id,
...
if __name__ == '__main__':
    ...
    logger.info("⚡ Consensus Threshold: 3/6 strategies")
