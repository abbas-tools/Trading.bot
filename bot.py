...
# 8. Self-Ping Keep-Alive for Render (prevents free tier spin-down)
# 9. Bitnode Monitoring (Bitget API connectivity health check)
# 10. Multi-Exchange Data Fetching (Binance, Coinbase, Kraken) for analysis
# ============================================================
import eventlet
...
# 🔥 WIN-RATE BLOCK DISABLED (set to 0)
MIN_WIN_RATE = 0.0
MIN_LOT_SIZE = 0.001
LOT_SIZE_STEP = 0.001
MIN_ATR_PERCENT = 0.5
...
# ============================================================
# DATABASE - UPGRADED WITH NEW COLUMNS & THREAD SAFE
# ============================================================
class DatabaseManager:
    ...
    def get_connection(self):
        if not hasattr(self._local, 'conn') or self._local.conn is None:
            # ✅ Added timeout to avoid locking issues
...
# ============================================================
def keep_alive():
    """Pings the app's /health endpoint every 5 minutes to prevent Render free tier spin-down."""
    public_url = os.environ.get('PUBLIC_URL')
    if not public_url:
        logger.info("ℹ️  No PUBLIC_URL set, keep-alive thread not started.")
        return
    logger.info(f"🔄 Keep-alive thread started – pinging {public_url}/health every 5 minutes")
    while True:
        try:
...
# Start keep-alive thread if PUBLIC_URL is set (typically on Render)
...
# ============================================================
# ADDED: run_async_fetch for async-like fetching (used by MarketCapAltcoinSeasonStrategy)
# ============================================================
def run_async_fetch(url):
    """Synchronous wrapper for async-like fetch (used by MarketCapAltcoinSeasonStrategy)."""
    try:
...
# ============================================================
# ADDED: PriceFetcher class for live price updates
# ============================================================
class PriceFetcher:
    """Simple price fetcher to provide live prices without breaking."""
    def __init__(self):
        self.prices = {}
        self._running = False
        self._thread = None
    ...
    def start(self):
        ...
        self._thread.start()
        logger.info("📈 PriceFetcher started (dummy)")
    ...
    def _run(self):
        # Dummy loop – can be extended to actually fetch prices if needed
        while self._running:
            time.sleep(1)
...
# Instantiate globally so other parts can use it
price_fetcher = PriceFetcher()

# ============================================================
# BITNODE (Bitget Connectivity Health Check)
# ============================================================
def check_bitget_node():
    """Check Bitget API connectivity and latency."""
    try:
        start = time.time()
        response = requests.get(f"{BITGET_REST_URL}/api/v2/public/time", timeout=5)
        ...
        return {'status': 'error', 'latency_ms': latency, 'error': 'Invalid response'}
    except Exception as e:
        return {'status': 'offline', 'error': str(e), 'timestamp': time.time()}

# ============================================================
# MULTI-EXCHANGE DATA FETCHING (Analysis Only)
# ============================================================
# Symbol mapping for different exchanges
def get_exchange_symbol(symbol: str, exchange: str) -> str:
    """Convert standard symbol (e.g., BTCUSDT) to exchange-specific format."""
    if exchange == 'bitget':
        return symbol
    elif exchange == 'binance':
        return symbol  # Binance uses same format BTCUSDT
    elif exchange == 'coinbase':
        # Coinbase uses BTC-USD, ETH-USD, etc. Remove USDT suffix and replace with -USD
        ...
        # Kraken uses XBT/USD for BTC, ETH/USD for ETH, etc.
        # For simplicity, we'll map common ones; else return as is
        mapping = {
            'BTCUSDT': 'XBT/USD',
            'ETHUSDT': 'ETH/USD',
            'SOLUSDT': 'SOL/USD',
            ...
            'NEARUSDT': 'NEAR/USD',
            'APTUSDT': 'APT/USD',
            'ARBUSDT': 'ARB/USD',  # Actually ARB/USD? Check
            'OPUSDT': 'OP/USD',
            'INJUSDT': 'INJ/USD',
            'SUIUSDT': 'SUI/USD',
            'SEIUSDT': 'SEI/USD',
...
def fetch_candles_binance(symbol, interval, limit=100):
    ...
    try:
        ...
        logger.debug(f"Binance candle fetch error: {e}")
    return None

def fetch_candles_coinbase(symbol, interval, limit=100):
    """Fetch candles from Coinbase."""
    try:
        # Coinbase uses different granularity: 60, 300, 900, 3600, 21600, 86400
        granularity_map = {
            '1m': 60,
            '5m': 300,
            '15m': 900,
        ...
        if response.status_code == 200:
            ...
            df['timestamp'] = df['timestamp']  # already seconds
            df = df[['timestamp', 'open', 'high', 'low', 'close', 'volume']]
            for col in ['open', 'high', 'low', 'close', 'volume']:
                df[col] = df[col].astype(float)
            df = df.sort_values('timestamp').reset_index(drop=True)
        ...
        logger.debug(f"Coinbase candle fetch error: {e}")
    return None

def fetch_candles_kraken(symbol, interval, limit=100):
    ...
    try:
        ...
        url = f"{KRAKEN_REST_URL}/public/OHLC"
        params = {
            'pair': symbol,
            'interval': kraken_interval,
            'since': None  # will get last 100 candles
        }
        response = requests.get(url, params=params, timeout=10)
        if response.status_code == 200:
            data = response.json()
        ...
        logger.debug(f"Kraken candle fetch error: {e}")
    return None

_candle_fail_info = {}  # key -> (fail_count, last_fail_time)
CANDLE_FAIL_LIMIT = 3
CANDLE_FAIL_TIMEOUT = 3600  # 1 hour
...
def fetch_candles(symbol, granularity="1H", limit=100):
    ...
    # Check if symbol is marked as bad for this granularity
    key = f"{symbol}_{granularity}"
    info = _candle_fail_info.get(key)
    if info:
        fail_count, last_fail = info
        if fail_count >= CANDLE_FAIL_LIMIT and (time.time() - last_fail) < CANDLE_FAIL_TIMEOUT:
            logger.debug(f"Skipping {symbol} {granularity} (marked bad)")
            return None
    
    # Try Bitget first (primary)
    df = _fetch_candles_bitget(symbol, granularity, limit)
    if df is not None:
        _candle_fail_info[key] = (0, 0)
        cache_set(cache_key, json.dumps(df.to_dict('records')), ttl=30)
        return df
    else:
        # Bitget failed, try other exchanges
        logger.debug(f"Bitget failed for {symbol} {granularity}, trying Binance...")
        # Convert symbol for Binance (same format)
        binance_symbol = get_exchange_symbol(symbol, 'binance')
        df = fetch_candles_binance(binance_symbol, granularity, limit)
        if df is not None:
            _candle_fail_info[key] = (0, 0)
            cache_set(cache_key, json.dumps(df.to_dict('records')), ttl=30)
            return df
        
        logger.debug(f"Binance failed for {symbol} {granularity}, trying Coinbase...")
        coinbase_symbol = get_exchange_symbol(symbol, 'coinbase')
        df = fetch_candles_coinbase(coinbase_symbol, granularity, limit)
        if df is not None:
            _candle_fail_info[key] = (0, 0)
            cache_set(cache_key, json.dumps(df.to_dict('records')), ttl=30)
            return df
        
        logger.debug(f"Coinbase failed for {symbol} {granularity}, trying Kraken...")
        kraken_symbol = get_exchange_symbol(symbol, 'kraken')
        df = fetch_candles_kraken(kraken_symbol, granularity, limit)
        if df is not None:
            _candle_fail_info[key] = (0, 0)
            cache_set(cache_key, json.dumps(df.to_dict('records')), ttl=30)
            return df
        
        # All exchanges failed, mark as bad
        _candle_fail_info[key] = (fail_count + 1 if info else 1, time.time())
        logger.warning(f"All exchanges failed for {symbol} {granularity}, marking bad")
        return None

def _fetch_candles_bitget(symbol, granularity, limit):
    """Internal Bitget candle fetch with retries."""
    ...
    for attempt in range(3):
        try:
            ...
            if response and response.get('code') == '00000':
                ...
                logger.warning(f"Bitget candle fetch attempt {attempt+1} failed for {symbol} {granularity}")
        except Exception as e:
            logger.warning(f"Bitget candle fetch attempt {attempt+1} error: {e}")
        time.sleep(1.5)
    return None
...
# ============================================================
# FEAR & GREED / BTC TREND (unchanged)
# ============================================================
...
# ============================================================
# MARKET REGIME DETECTOR (unchanged)
# ============================================================
...
# ============================================================
# ULTIMATE TREND ANALYZER (unchanged)
# ============================================================
...
class AdaptiveLearner:
    ...
    def update_strategy_performance(self, strategy_name: str, is_win: bool):
        ...
        try:
            with db_manager.get_connection() as conn:
                ...
                conn.commit()
                self.load_strategy_weights()
                self.strategy_weights_cache = self.strategy_weights
    ...
    def get_strategy_weight(self, strategy_name: str) -> float:
        """Get current weight for a strategy with cache"""
        # Update cache every 60 seconds
        if time.time() - self.last_weight_update > 60:
            self.load_strategy_weights()
            self.strategy_weights_cache = self.strategy_weights
            self.last_weight_update = time.time()
        if strategy_name in self.strategy_weights_cache:
            return self.strategy_weights_cache[strategy_name]['weight']
        return 1.0
...
# ============================================================
# STRATEGY 8: MARKET CAP & ALTCOIN SEASON (Async-enabled) – unchanged
# ============================================================
class MarketCapAltcoinSeasonStrategy:
    ...
    def _fetch_total_market_cap_async(self):
        """Asynchronously fetch total market cap with retry"""
        try:
...
# ============================================================
# EXISTING 7 STRATEGIES (Preserved, unchanged)
# ============================================================
...
class ConsensusEngine:
    ...
    def get_strategy_weights(self):
        """Return weights for each strategy from learner (cached)"""
        weights = {}
...
# ============================================================
# RISK MANAGER - FULLY UPGRADED (Complete)
# ============================================================
class RiskManager:
    ...
    def check_circuit_breaker(self) -> Tuple[bool, str]:
        ...
        if self.daily_pnl <= -(self.capital * DAILY_LOSS_LIMIT):
            return False, f"Daily loss limit ${self.capital * DAILY_LOSS_LIMIT:.2f} reached"
    ...
    def calculate_position_size_advanced(self, capital: float, confidence: float,
        ...
        try:
            ...
            if avg_atr > 0:
                ...
                if vol_ratio > 1.2:
                    base_risk *= (1.0 / vol_ratio)
                elif vol_ratio < 0.8:
                    base_risk *= (1.0 + (0.8 - vol_ratio))
    ...
    def calculate_position_size(self, capital: float, entry: float, stop_loss: float,
        ...
        """Simpler version using entry and stop."""
        try:
            ...
            max_allowed = (capital * 0.5) / entry if entry > 0 else MIN_LOT_SIZE
    ...
    def calculate_stop_loss_take_profit(self, entry: float, side: str, df: pd.DataFrame) -> Tuple[float, float, List[float], List[float]]:
        """Return SL, TP1, TP list, fractions."""
        try:
            atr = calculate_atr(df)
            ...
            logger.error(f"SL/TP calc error: {e}")
            if side == 'BUY':
    ...
    def check_trading_allowed(self, current_capital: float) -> Tuple[bool, str]:
        self.check_daily_reset()
        ok, msg = self.check_circuit_breaker()
        if not ok:
            return False, msg
...
# ============================================================
# ADDED: format_price helper for adaptive decimal display
# ============================================================
def format_price(price: float) -> str:
    """Return price as string with appropriate decimal places."""
...
class QuantumWhaleBot:
    ...
    def generate_signal_message(self, symbol: str, side: str, confidence: float,
        ...
        # 🔥 IMPROVED: Include ALL strategy details
        strategy_details = "\n".join([
            f"  • {s['name']}: {s['signal']} ({s['confidence']:.0f}%) - {s['details']} (weight: {s.get('weight',1.0):.2f})"
            for s in strategies if s['signal'] != 'NEUTRAL'  # only show non-neutral
        ])
        if not strategy_details:
            strategy_details = "  • No active strategy signals"
        ...
        # Use format_price for entry, sl, tp
...
"""
    ...
    def analyze_and_trade(self):
        ...
        # ✅ WIN-RATE BLOCK REMOVED (MIN_WIN_RATE set to 0)
        ...
        # ✅ Update top movers only if interval elapsed (handled inside class)
        ...
        for symbol, score in top_coins[:10]:
            for tf in trade_timeframes:
                try:
                    ...
                    # ✅ Prevent duplicate TF trades on same symbol
                    ...
                    # ✅ PER-COIN TREND FILTER
                    coin_trend = self.trend_analyzer.analyze_trend(symbol)
                    weekly_trend_coin = coin_trend.get('overall', 'NEUTRAL')
                    trend_strength = coin_trend.get('strength', 0)
                    
                    signal_side = consensus['signal']
                    if weekly_trend_coin in ['STRONG_BULLISH', 'BULLISH']:
                        if signal_side == 'SELL':
                            logger.info(f"🚫 {symbol}: Weekly trend BULLISH, SELL blocked")
                            continue
                    elif weekly_trend_coin in ['STRONG_BEARISH', 'BEARISH']:
                        if signal_side == 'BUY':
                            logger.info(f"🚫 {symbol}: Weekly trend BEARISH, BUY blocked")
                            continue
                    else:  # NEUTRAL trend – allow but require higher confidence
                        if trend_strength < 40 and consensus['confidence'] < 75:
                            logger.info(f"🚫 {symbol}: Weak trend ({trend_strength:.0f}) and confidence {consensus['confidence']:.0f} < 75")
                            continue
                    ...
                    # -------- NEW VALIDATION --------
                    # Ensure stop-loss and take-profit are not too close to entry
                    risk = abs(current_price - sl)
                    reward = abs(tp - current_price)
                    min_distance = max(1e-8, current_price * 0.001)  # at least 0.1% or absolute 1e-8
                    if risk < min_distance or reward < min_distance:
                        logger.debug(f"SL/TP too close to entry for {symbol} (risk={risk:.8f}, min={min_distance:.8f})")
                        continue
                    # Ensure sl and tp are not equal to current_price
                    if sl == current_price or tp == current_price:
                        logger.debug(f"SL or TP equals entry for {symbol}, adjusting slightly")
                        if side == 'BUY':
                            sl = current_price * 0.999
                            tp = current_price * 1.002
                        else:
                            sl = current_price * 1.001
                            tp = current_price * 0.998
                    # -------- END VALIDATION --------
                    ...
                    # Use coin's weekly trend for filtering, but for message we can use coin's trend
                    weekly_trend_for_msg = weekly_trend_coin
                    ...
                    signal_text = self.generate_signal_message(
                        ...
                        ml_status, var_status, regime, sentiment, weekly_trend_for_msg,
                    ...
                    logger.info(f"📤 SIGNAL: {symbol} {consensus['signal']} @ {current_price:.4f} ({tf}) - Weighted Consensus | Trend: {weekly_trend_for_msg}")
                    ...
                    if len(self.active_trades) < MAX_ACTIVE_TRADES:
                        ...
                        if success:
                            ...
                            self.active_trades[symbol] = {
                                ...
                                'regime': regime,
                                'sentiment': sentiment,
                                'weekly_trend': weekly_trend_for_msg,
                                'sr_type': sr_type,
                                'is_breakout': is_breakout,
                                'breakout_type': breakout_type,
                                'macro_bias': macro_bias,
                            ...
                            self.log_trade(
                                ...
                                regime, self.daily_pnl, 0, weekly_trend_for_msg,
                            ...
                            self.adaptive_learner.log_learning_insight(
                                ...
                                consensus['confidence'], sentiment, regime, 0, weekly_trend_for_msg,
                                "NEUTRAL", macro_bias, tf,
                                market_cap_sentiment, funding_rate_sentiment
                            )
                            
                            logger.info(f"✅ EXECUTED: {symbol} {consensus['signal']} @ {current_price:.4f} ({tf}) - Weighted Consensus | Trend: {weekly_trend_for_msg}")
                except Exception as e:
    ...
    def manage_trades(self):
        ...
        for symbol in list(self.active_trades.keys()):
            try:
                ...
                current_capital = self.current_capital
                self.risk_manager.update_capital(self.current_capital - 10.0)  # PnL only
                ...
                for idx, level in enumerate(tp_levels):
                    if not level['executed']:
                        ...
                        if side == "BUY" and current_price >= target_price:
                            ...
                            current_capital = self.current_capital
                            self.risk_manager.update_capital(self.current_capital - 10.0)
    ...
    def send_realtime_update(self):
        ...
        macro_data = self.macro_indicator.get_gold_bias()
        bitnode = check_bitget_node()  # ✅ Added bitnode status
    ...
    def run(self):
        ...
        logger.info("  ✅ Bitnode Monitoring (Bitget API connectivity)")
        logger.info("  ✅ Multi-Exchange Data Fetching (Binance, Coinbase, Kraken)")
        logger.info("=" * 70)
        ...
        logger.info("=" * 70)
        logger.info("🔒 Keep-alive: " + ("ACTIVE" if os.environ.get('PUBLIC_URL') else "INACTIVE (set PUBLIC_URL for 24/7 operation)"))
...
if __name__ == '__main__':
    ...
    logger.info("🔄 Self-Ping Keep-Alive: " + ("ACTIVE" if os.environ.get('PUBLIC_URL') else "INACTIVE (set PUBLIC_URL)"))
    logger.info("📡 Bitnode Monitoring: ENABLED")
    logger.info("🌐 Multi-Exchange Analysis: Binance, Coinbase, Kraken")
    logger.info("=" * 70)
    ...
    logger.info("🤖 UptimeRobot can ping /health every 5 minutes (recommended)")
