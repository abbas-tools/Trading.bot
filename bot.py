...
# ============================================================
# PRODUCTION SERVER - Using Gunicorn or Direct Run
# ============================================================

# This is the application object for Gunicorn
# Gunicorn will look for 'app' variable
# To run: gunicorn -k eventlet -w 1 bot:app
...
if __name__ == '__main__':
    ...
    # For Railway - use PORT from environment
    port = int(os.environ.get('PORT', 5000))
    
    # Check if running under gunicorn
    if 'gunicorn' in os.environ.get('SERVER_SOFTWARE', ''):
        logger.info("🚀 Running under Gunicorn - using app object")
        # Gunicorn will use the 'app' variable
    else:
        logger.info(f"🚀 Starting Flask development server on port {port}")
        # Use flask-socketio run only if not using gunicorn
        try:
            import eventlet
            eventlet.monkey_patch()
