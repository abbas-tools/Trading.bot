# ============================================================
# ULTIMATE QUANTUM WHALE v17.1 - TOP TRADER EDITION (ENHANCED)
# ============================================================
import eventlet
...
TRAILING_STEP = 0.5
REQUIRE_BTC_ALIGNMENT = False  # ✅ DISABLED: Allow independent altcoin trades
BTC_SYMBOL = "BTCUSDT"
...
def init_db():
    with db_manager.get_connection() as conn:
        ...
        # ✅ v17.1: Learning logs table for self-learning
        ...
        conn.commit()
    logger.info("✅ Database initialized with v17.1 Enhanced features")
...
# ============================================================
# v17 FEATURE 6: NEWS FILTER / ECONOMIC CALENDAR (🔇 DISABLED)
# ============================================================
class NewsFilter:
    def __init__(self):
    ...
    def check_high_impact_news(self) -> Tuple[bool, str]:
        """✅ DISABLED: Bot trades 24/7 during news for maximum opportunity"""
        # Original news checking logic is completely bypassed
        # Bot now trades regardless of news events
        return False, "News filter disabled - Trading allowed 24/7"

# ============================================================
# v17.1 FEATURE: TOP GAINERS/LOSERS TRACKER (24H)
# ============================================================
class TopMoversTracker:
    ...
    def update(self):
        """Fetch 24h price changes for all symbols and rank them"""
        try:
            movers = []
    ...
    def get_best_opportunities(self, count=5):
        """Combined score for best trading opportunities"""
        opportunities = []
...
# ============================================================
# v17.1 FEATURE: ENHANCED MARKET SENTIMENT ANALYZER
# ============================================================
class EnhancedSentimentAnalyzer:
    ...
    def analyze_market_sentiment(self, df: pd.DataFrame, symbol: str) -> Dict:
        """Advanced sentiment analysis combining multiple factors"""
        try:
            if df is None or len(df) < 50:
    ...
    def get_market_mood(self) -> str:
        """Overall market mood based on historical sentiment"""
...
# ============================================================
# v15 ADAPTIVE LEARNING (Intact + Enhanced)
# ============================================================
class AdaptiveLearner:
    ...
    # ✅ v17.1: Enhanced self-learning with sentiment and regime context
    def log_learning_insight(self, symbol: str, action: str, outcome: str, 
                             confidence: float, sentiment: str, regime: str, pnl: float = 0):
        """Log insights for self-learning with full context"""
        try:
            with db_manager.get_connection() as conn:
                conn.execute("""
                    INSERT INTO learning_logs (timestamp, symbol, action, outcome, confidence, sentiment, regime, pnl)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (time.strftime('%Y-%m-%d %H:%M:%S'), symbol, action, outcome, confidence, sentiment, regime, pnl))
                conn.commit()
            ...
            logger.info(f"🧠 Learning insight: {symbol} {action} → {outcome} (Conf: {confidence:.0f}%, Sentiment: {sentiment})")
...
# ============================================================
# CONSENSUS ENGINE (v14 + v15 + v16 + v17 + v17.1 Enhanced)
# ============================================================
class ConsensusEngine:
    ...
    def analyze(self, df: pd.DataFrame, symbol: str, positions: Dict) -> Dict:
        ...
        # ✅ v17.1: Enhanced Sentiment Analysis
        sentiment_result = self.sentiment_analyzer.analyze_market_sentiment(df, symbol)
        ...
        # ✅ v17.1: Sentiment-based confidence adjustment
        if sentiment_result['sentiment'] == 'BULLISH':
            for r in results:
                if r['signal'] == 'BUY':
                    r['confidence'] = min(95, r['confidence'] * 1.1)
        ...
        if buy_count >= self.consensus_threshold:
            ...
            # ✅ v17.1: Sentiment boost
            if sentiment_result['sentiment'] == 'BULLISH':
                final_confidence = min(95, final_confidence * 1.08)
            ...
            reason = f"{buy_count}/4 strategies agree on BUY (Regime: {current_regime}, Sentiment: {sentiment_result['sentiment']}, Weight: {pattern_weight:.2f}, {ml_status})"
            ...
            reason = f"{sell_count}/4 strategies agree on SELL (Regime: {current_regime}, Sentiment: {sentiment_result['sentiment']}, Weight: {pattern_weight:.2f}, {ml_status})"
        else:
...
# ============================================================
# RISK MANAGER (v17 Professional + News Bypass)
# ============================================================
class RiskManager:
    def __init__(self):
    ...
    def check_trading_allowed(self, current_capital: float) -> Tuple[bool, str]:
        """✅ v17.1: Trading allowed - News filter bypassed for 24/7 trading"""
        ...
        # 2. News Filter - ✅ BYPASSED for 24/7 trading
        # news_ok, news_reason = self.news_filter.check_high_impact_news()
        # if news_ok:
        #     return False, f"News Filter: {news_reason}"
        # News filter is completely disabled - trading allowed anytime
...
# ============================================================
# MAIN TRADING BOT (v17.1 Enhanced)
# ============================================================
...
class QuantumWhaleBot:
    ...
    def generate_signal_message(self, symbol: str, side: str, confidence: float,
        ...
        return f"""
🐋 *QUANTUM WHALE v17.1 ENHANCED SIGNAL*
...
"""
    ...
    def analyze_and_trade(self):
        ...
        try:
            ...
            # ✅ v17.1: Enhanced coin scoring with sentiment + movers
            coin_scores = {}
            for symbol in ALL_SYMBOLS[:15]:
                try:
                    ...
                    score = (volume_ratio * 0.3 + 
                            atr_percent * 0.2 + 
                            abs(df['close'].pct_change().iloc[-1]) * 0.2 +
                            sentiment_score * 0.2 +
                            mover_bonus * 0.1)
                    ...
                    if score > 1.0:
                        logger.info(f"🔥 High potential: {symbol} (Score: {score:.2f}, Sentiment: {sentiment_result['sentiment']})")
            ...
            for symbol in trading_symbols[:10]:
                try:
                    ...
                    sentiment = consensus.get('sentiment_result', {}).get('sentiment', 'NEUTRAL')
                    ...
                    signal_text = self.generate_signal_message(
                        ...
                        ml_status, var_status, regime, sentiment
                    )
                    
                    safe_telegram_send(signal_text)
                    logger.info(f"📤 SIGNAL: {symbol} {consensus['signal']} @ {current_price:.4f} | R:R 1:{actual_rr:.1f} | Regime: {regime} | Sentiment: {sentiment}")
        ...
        for symbol in list(self.active_trades.keys()):
            try:
                ...
                if side == "BUY" and current_price >= tp:
                    ...
                    self.adaptive_learner.log_learning_insight(symbol, side, "WIN", trade.get('confidence', 0), sentiment, regime, profit)
                    safe_telegram_send(f"🎯 TP HIT {symbol}: +${profit:.4f} | Capital: ${self.current_capital:.2f} | Sentiment: {sentiment}")
                ...
                # SL Check
                    ...
                    self.adaptive_learner.log_learning_insight(symbol, side, "LOSS", trade.get('confidence', 0), sentiment, regime, -loss)
                    safe_telegram_send(f"🛑 SL HIT {symbol}: -${loss:.4f} | Capital: ${self.current_capital:.2f} | Sentiment: {sentiment}")
    ...
    def run(self):
        ...
        logger.info("🐋 Quantum Whale v17.1 - Enhanced Top Trader Edition")
        logger.info("=" * 70)
        logger.info("📊 ENHANCED Professional Features:")
        ...
        logger.info("  ✅ Auto-Skip after 2 Daily Losses")
        logger.info("  ✅ News Filter DISABLED (24/7 Trading)")
        logger.info("  ✅ BTC Alignment DISABLED (Independent Altcoins)")
        logger.info("  ✅ Top 24H Gainers/Losers Tracking")
        logger.info("  ✅ Enhanced Market Sentiment Analysis")
        logger.info("  ✅ Self-Learning Intelligence")
...
# ============================================================
# FLASK ROUTES (Updated for v17.1)
# ============================================================
bot = QuantumWhaleBot()
...
if __name__ == '__main__':
    logger.info("🚀 Starting Quantum Whale v17.1 - Enhanced Top Trader Edition...")
    logger.info("📊 All v10 + v12 + v14 + v15 + v16 + v17 + v17.1 features integrated!")
    ...
    logger.info("🔇 News Filter: DISABLED (24/7 Trading)")
    logger.info("📊 Top Movers Tracking: ENABLED")
