...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'quantum_whale_secret_2026')
...
try:
    ...
    logger.info("⚠️ Using threading mode (not recommended for production)")
...
# --- PRODUCTION SERVER ---
def start_server():
    """Start the production server with eventlet or gunicorn"""
    port = int(os.environ.get('PORT', 5000))
    host = '0.0.0.0'
    
    # Try using eventlet first
    try:
        import eventlet
        eventlet.monkey_patch()
        logger.info(f"🚀 Starting eventlet server on {host}:{port}")
        socketio.run(app, host=host, port=port, debug=False)
    except ImportError:
        # Fallback to werkzeug (not recommended for production)
        logger.warning("⚠️ eventlet not available, using werkzeug (not production ready)")
