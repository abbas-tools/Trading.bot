...
# ============================================================
# TOP 10 HIGH-PERFORMANCE ALTCOINS FOR SMART PICKING
# ============================================================
...
# Main symbols + altcoins
MAIN_SYMBOLS = ["XAUUSDT", "BTCUSDT", "ETHUSDT"]
ALL_SYMBOLS = MAIN_SYMBOLS + TOP_ALTCOINS

# ============================================================
# ENHANCED POSITION SIZING - 30% PORTFOLIO MANAGEMENT
# ============================================================

# Position sizing configuration
MIN_LOT_SIZE = 0.001
MAX_POSITION_SIZE = 0.05  # Increased for 30% portfolio allocation
LOT_SIZE_STEP = 0.001
...
RISK_REWARD_RATIO = 3.0  # 1:3 Risk:Reward (min 1:2)
...
def calculate_smart_position_size(capital, kelly_fraction, atr, current_price, confidence_score=1.0):
    """Enhanced position sizing with 30% portfolio allocation"""
    try:
        ...
        # Calculate 30% portfolio allocation
        ...
        # Risk-based position (3% of capital)
        ...
        final_position *= POSITION_SIZE_MULTIPLIER
        ...
        if final_position < MIN_LOT_SIZE:
            ...
            logger.debug(f"⚠️ Position increased to minimum lot: {MIN_LOT_SIZE}")
...
# ============================================================
# SMART COIN SELECTOR - AI Market Analysis
# ============================================================

class SmartCoinSelector:
    """Intelligent coin selection with multi-factor analysis"""
    ...
    def analyze_coins(self):
        """Analyze all coins and rank them by performance potential"""
        try:
            scores = {}
            ...
            for symbol in ALL_SYMBOLS:
                try:
                    ...
                    # 4. Volatility Score (healthy volatility)
                    ...
                    # Price change (24h)
                    price_24h = ((close[-1] - close[-25]) / close[-25]) * 100 if len(close) >= 25 else 0
    ...
    def get_best_coins(self, limit=5):
        ...
        return [c[0] for c in self.top_coins[:limit] if c[1]['score'] > 0.5]

    def get_coin_score(self, symbol):
        """Get individual coin score"""
        if symbol in self.coin_scores:
            return self.coin_scores[symbol]
        return None

# Initialize smart coin selector
smart_selector = SmartCoinSelector()

# ============================================================
# REAL-TIME BITGET BALANCE WITH PORTFOLIO TRACKING
# ============================================================
...
MIN_ATR_PERCENT = 0.3
MAX_ATR_PERCENT = 3.0  # Increased for altcoins
...
ATR_TP_MULTIPLIER_BASE = 3.0  # 1:3 Risk:Reward
...
logger.info(f"📈 Tracking {len(ALL_SYMBOLS)} coins including top altcoins")
logger.info(f"🧠 Smart Coin Selection: ENABLED")
...
def calculate_position_size(capital, kelly, atr, price, confidence_score=1.0):
    """Wrapper for enhanced position sizing"""
...
def fetch_candles(symbol, granularity="1H", limit=100):
    ...
    try:
        ...
        if data.get('code') == '00000':
            ...
            # Update live price
            if 'live_prices' in globals() and symbol in live_prices:
...
# ============================================================
# MODIFIED analyze_and_trade() WITH SMART COIN SELECTION
# ============================================================
...
def analyze_and_trade():
    ...
    # Smart coin selection - find best coins
    top_coins = smart_selector.get_best_coins(limit=5)
    
    # Combine with main symbols
    trading_symbols = MAIN_SYMBOLS + top_coins
    trading_symbols = list(dict.fromkeys(trading_symbols))  # Remove duplicates

    # Check market sentiment (every 5 minutes)
    ...
    # Check network health (every 10 minutes)
    ...
    for symbol in trading_symbols[:8]:  # Limit to top 8 coins
        ...
        try:
            ...
            if coin_score and coin_score.get('score', 0) < 0.4:
                logger.info(f"⏭️ SKIPPED {symbol}: Low score {coin_score.get('score', 0):.2f}")
                continue
            ...
            # Calculate position size with 30% portfolio allocation
            size = calculate_smart_position_size(portfolio_value, kelly_fraction, atr, current_price, confidence_factor)

            # Calculate leverage (max 10x, based on confidence)
            leverage = min(MAX_LEVERAGE, max(1, int(confidence_score / 10)))
            ...
            if success:
                ...
                signal_text = (
                    ...
                    f"📈 Portfolio: `{PORTFOLIO_ALLOCATION*100:.0f}%`\n"
                ...
                logger.info(f"✅ {mode} order: {symbol} {side} @ {current_price:.2f} (Confidence: {confidence_score:.0f}%, Leverage: {leverage}x, Kelly: {kelly_fraction:.2%})")

        except Exception as e:
            logger.error(f"❌ Analysis error {symbol}: {e}")

# ============================================================
# BITNODES (Bitcoin Network Data)
# ============================================================
...
def run_bot():
    ...
    logger.info(f"💰 Portfolio Allocation: {PORTFOLIO_ALLOCATION*100}% per trade")
    logger.info(f"⚡ Risk:Reward: 1:{RISK_REWARD_RATIO}")
    logger.info(f"📈 Tracking {len(ALL_SYMBOLS)} coins with smart selection")
    logger.info(f"🧠 Smart Intelligence: ENABLED")
...
def handle_connect():
    ...
    emit('connected', {
        ...
        'max_leverage': MAX_LEVERAGE,
        'portfolio_allocation': PORTFOLIO_ALLOCATION * 100,
        'top_coins': smart_selector.top_coins[:3]
    })
...
def get_chart_data():
    try:
        chart_data = {}
        for symbol in ALL_SYMBOLS[:15]:  # Limit for performance
            ...
            if candle_data.get(symbol) and candle_data[symbol].get('close'):
                ...
                chart_data[symbol] = {
                    ...
                    "patterns": [], "current_price": 0, "change_24h": 0, "sma_20": 0, "mode": TRADING_MODE,
                    "has_data": False
                }
...
def get_stats():
    ...
    stats['top_coins'] = smart_selector.top_coins[:5]
    stats['filters'] = {
        "atr": f"{MIN_ATR_PERCENT}%-{MAX_ATR_PERCENT}%",
        "volume": f"{MIN_VOLUME_MULTIPLIER}x",
        "rsi": f"{RSI_OVERSOLD}-{RSI_OVERBOUGHT}",
...
logger.info("✅ Starting Quantum Whale Terminal v6.0...")
logger.info(f"📊 Mode: {TRADING_MODE}")
