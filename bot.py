...
def get_statistics():
    """Get trading statistics with proper win rate calculation"""
    try:
        with db_manager.get_connection() as conn:
            cursor = conn.cursor()
            
            # Get total trades, wins, losses
            cursor.execute("""
                SELECT 
                    COUNT(*) as total,
    ...
    return {"total": 0, "wins": 0, "losses": 0, "win_rate": 0, "total_pnl": 0, "avg_confidence": 0, "avg_leverage": 0}
...
# ============================================================
# ANALYZE AND TRADE - LIMITED TO 3 ACTIVE TRADES
# ============================================================
...
def analyze_and_trade():
    ...
    try:
        ...
        # ============================================================
        # CHECK ACTIVE TRADES LIMIT - ONLY 3 TRADES!
        # ============================================================
        active_count = len(active_trades)
        if active_count >= MAX_ACTIVE_TRADES:
            logger.info(f"⏸️ Max active trades reached ({active_count}/{MAX_ACTIVE_TRADES}) - waiting for TP/SL")
            return
        ...
        # Process symbols - only take up to MAX_ACTIVE_TRADES
        trades_taken = 0
        for symbol in trading_symbols[:15]:  # Check top 15 coins
            # Stop if we already have MAX_ACTIVE_TRADES
            ...
            try:
                ...
                logger.info(f"📊 TRADE SIGNAL: {symbol} {side} @ {current_price:.2f} (Score: {score_value:.2f}, Confidence: {confidence_score:.0f}%, Leverage: {leverage}x, Size: {size:.4f})")
        ...
        if trades_taken > 0:
            logger.info(f"✅ Took {trades_taken} new trades. Active: {len(active_trades)}/{MAX_ACTIVE_TRADES}")
